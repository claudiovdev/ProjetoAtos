...
public class RecrutadorController {
	...
	public void deletarRecrutador(@PathVariable Long id) {
		recrutadorRepository.deleteById(id);
	}
	
	@ApiOperation(value= "Atualiza um recrutador")
	public Recrutador atualizarRecrutador(@RequestBody Recrutador recrutadorNovo,@PathVariable Long id) {
		return recrutadorRepository.findById(id).map(recrutadorAntigo -> {
			recrutadorAntigo.setNome(recrutadorNovo.getNome());
			recrutadorAntigo.setSobrenome(recrutadorNovo.getSobrenome());
			recrutadorAntigo.setEmail(recrutadorNovo.getEmail());
			Recrutador update = recrutadorRepository.save(recrutadorAntigo);
			return update;
		}).orElse(null);
	}
